...
class ExperiencesPresenterImpl(
  ...
  override def onContinue(): Unit =
    val experienceTuples = model.experiences.toList.map(experience => (experience.title(), experience.organization(), experience.duration(), experience.location(), experience.description(), experience.skills()))
